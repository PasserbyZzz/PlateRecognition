...
class CardPredictor:
    ...
    def predict_plate_image(self, car_pic, resize_rate=1):
        ...
        cv2.imwrite('./img_edge2.png', img_edge1)
...
#     c.train_svm()
#     # 识别到的字符、定位的车牌图像、车牌颜色
#     r, roi, color = c.predict("./test/12.jpg")
#     print("r:", r)
#     print("color:", color)

if __name__ == '__main__':
    locator = CardPredictor()
    colors, card_imgs = locator.predict_plate_image("./test/2.jpg")

    for index, (color, card_img) in enumerate(zip(colors, card_imgs)):
        print(color)
        if card_img is not None:
            cv2.imshow(f"plate_{index}", card_img)

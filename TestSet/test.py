...
class PlatesLocator:
        ...
        for rect in car_contours:
            ...
            if left_point[1] <= right_point[1]:
                ...
                if (right_point[0] == heigth_point[0]):
                    card_img = old_img[int(left_point[1]):int(heigth_point[1]),
            ...
            # 负角度
                ...
                if (left_point[0] == heigth_point[0]):
                    card_img = old_img[int(right_point[1]):int(heigth_point[1]),
    ...
    def separate_characters(self, card_img, color="blue"):
        ...
        # 判断是否是左侧车牌边缘
        if wave_peaks and wave_peaks[0][1] - wave_peaks[0][0] < (wave_peaks[-1][1] - wave_peaks[-1][0]) / 3 and \
                wave_peaks[0][0] == 0:
            wave_peaks.pop(0)
...
#############################################################################################################
    ########################这里是整个脚本的输入端口###############################
    plate_imgs, plate_colors = locator.locate_plates("2.jpg")#################在这里输入
